...
Filter::Filter(vector<SHP_Caller> callers_) : vecCallers(callers_)
{
	tracks = vecCallers.size();
	...
	for (int i = 0; i < tracks; ++i)
	{
		...
		{
			LOG::Log(LOG::fatal, "ERRORS", "MSCNF: Filter ERROR");
			exit(-1);
		}
	}
}
...
int Filter::InitFilterGraph(int for_client_)
{
	...
	for (int i = 0; i < tracks; ++i)
	{
		...
		if (!vecCallers[i]->iccx->channel_layout)
		{
			vecCallers[i]->iccx->channel_layout = av_get_default_channel_layout(vecCallers[i]->iccx->channels);
		}

		string arg1 = str(template_arg_create_graph
			%vecCallers[i]->iccx->sample_rate
			%av_get_sample_fmt_name(vecCallers[i]->iccx->sample_fmt)
			% vecCallers[i]->iccx->channel_layout
			);

...
Filter::Filter(vector<SHP_Point> points_) : vecPoint(points_)
{
	tracks = vecPoint.size();
	...
	for (int i = 0; i < tracks; ++i)
	{
		...
		{
			cout << "\nFilter error";
			LOG::Log("fatal", "Errors", "Filter::Filter fatal error");
			system("pause");
		}
	}
}
...
int Filter::InitFilterGraph(int for_client_)
{
	...
	for (int i = 0; i < tracks; ++i)
	{
		...
		if (!vecPoint[i]->iccx->channel_layout)
		{
			vecPoint[i]->iccx->channel_layout = av_get_default_channel_layout(vecPoint[i]->iccx->channels);
		}

		string arg1 = str(template_arg_create_graph
			%vecPoint[i]->iccx->sample_rate
			%av_get_sample_fmt_name(vecPoint[i]->iccx->sample_fmt)
			% vecPoint[i]->iccx->channel_layout
			);

...
MediaFile::MediaFile(string filename_)
{
	...
	if (OpenFile() == -1)
	{
		LOG::Log(LOG::fatal, "ERRORS", "MSANN: MediaFile ERROR 1");
...
void MediaFile::CutPackets(vector<SHP_PACKET> file_)
{
	for (int i = 0; i < (int)file_.size(); ++i)
	{
		...
		{
			SHP_PACKET cut_packet = make_shared<PACKET>(file_[i]->Data() + j * 160, 160);
...
//*///------------------------------------------------------------------------------------------
//*///------------------------------------------------------------------------------------------
vector<SHP_MediaFile> MusicStore::data;
//*///------------------------------------------------------------------------------------------
...
SHP_MediaFile MusicStore::OpenNewFile(string filename_)
{
	...
	if (new_file->error == "")
	{
		data.push_back(new_file); //single file DEBUG
		return new_file;
	}
	else return nullptr;
}

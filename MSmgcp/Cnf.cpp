...
{
	BOOST_LOG_SEV(lg, trace) << "Cnf::Cnf with eventID=" << event_id_;
	vecPoints.push_back(point_);
	eventID = event_id_;
	BOOST_LOG_SEV(lg, trace) << "Cnf::Cnf END";
}
//*///------------------------------------------------------------------------------------------
//*///------------------------------------------------------------------------------------------
void Cnf::AddPoint(SHP_Point point_)
{
	BOOST_LOG_SEV(lg, trace) << "Cnf::AddPoint with eventID=" << eventID;
	vecPoints.push_back(point_);
	Process(point_);
	BOOST_LOG_SEV(lg, trace) << "Cnf::AddPoint END";
}
//*///------------------------------------------------------------------------------------------
//*///------------------------------------------------------------------------------------------
bool Cnf::DeletePoint(SHP_Point point_)
{
	BOOST_LOG_SEV(lg, trace) << "Cnf::DeletePoint with eventID=" << eventID;
	vecPoints.erase(std::remove(vecPoints.begin(), vecPoints.end(), point_), vecPoints.end());
	if (vecPoints.size() == 0)
	{
		BOOST_LOG_SEV(lg, trace) << "Cnf::DeletePoint vecPoints.size() == 0";
		return true;
	}
	else 
	{ 
...
{
	BOOST_LOG_SEV(lg, trace) << "Cnf::Process with eventID=" << eventID;
	int active_points = GetNumOfActivePoints();
	if (state == false)
	{
		BOOST_LOG_SEV(lg, trace) << "Cnf::Process state == false";
		if (active_points == 2)
		{
			BOOST_LOG_SEV(lg, trace) << "Cnf::Process state == false, active_points == 2";
			state = true;
			BOOST_LOG_SEV(lg, trace) << "Cnf::Process point 0:" + vecPoints[0]->PrintPoint();
			BOOST_LOG_SEV(lg, trace) << "Cnf::Process point 1:" + vecPoints[1]->PrintPoint();
	...
	{
		BOOST_LOG_SEV(lg, trace) << "Cnf::Process state == true,";
		if (active_points == 1)
		{
			BOOST_LOG_SEV(lg, trace) << "Cnf::Process state == true, active_points == 1";
			state = false;
...
int Cnf::GetNumOfActivePoints()
{
	...
	BOOST_LOG_SEV(lg, trace) << "Cnf::GetNumOfActivePoints =" << result;
	return result;
}
//*///------------------------------------------------------------------------------------------
//*///------------------------------------------------------------------------------------------
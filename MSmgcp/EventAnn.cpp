...
{
	BOOST_LOG_SEV(lg, trace) << "EventAnn::CRCX(...) for eventID=" << mgcp_->data[MGCP::EventID];
...
{
	BOOST_LOG_SEV(lg, trace) << "EventAnn::RQNT(...) for eventID=" << mgcp_->data[MGCP::EventID];
	string action_result = CheckExistance(mgcp_);
...
{
	BOOST_LOG_SEV(lg, trace) << "EventAnn::DLCX(...) for " << mgcp_->data[MGCP::EventID];
	string action_result = CheckExistance(mgcp_);
	if (action_result == "-1" || action_result == "-2")
	{
	...
	FreeEventID(found_ann->eventID);
	BOOST_LOG_SEV(lg, trace) << "EventAnn::DLCX(...): FreeEventID(found_ann->eventID); DONE";
...
string EventAnn::CheckExistance(SHP_MGCP mgcp_)
{
	...
	else
	{
		for (auto& ann : vecAnn)
		{
			if (ann->eventID == mgcp_->data[MGCP::EventID])
			{
				if (ann->callID == mgcp_->data[MGCP::ParamC])
					return ann->eventID;
				else return "-2";

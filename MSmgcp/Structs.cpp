...
NETDATA::NETDATA()
{
	...
	for (int i = 0; i < maxS; ++i)
	{
		sockets[i].reset(new Socket(
			init_Params->data[IPar::outerIP + i],
			stoi(init_Params->data[IPar::outerPort + i]),
			GI(i)
			));
	}

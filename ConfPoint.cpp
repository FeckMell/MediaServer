...
void CConfPoint::ModifySDP()
{
	...
	NewSDP += cut_substr(SDP_, "s=", "\n") + "\n";
	NewSDP += cut_substr(SDP_, "c=", "\n") + "\n";
	NewSDP += cut_substr(SDP_, "t=", "\n") + "\n";
	NewSDP += cut_substr(SDP_, "m=", "\n") + "\n";
...
{
	AVCodec *x_codec;
	 x_codec = avcodec_find_decoder(AV_CODEC_ID_PCM_ALAW);
	iccx = avcodec_alloc_context3(x_codec);
	//avcodec_get_context_defaults3(iccx, x_codec);

	iccx->channels = 1;
	iccx->channel_layout = av_get_default_channel_layout(iccx->channels);
	iccx->sample_fmt = x_codec->sample_fmts[0];
	iccx->sample_rate = 8000;
	iccx->bit_rate = 8000;
	iccx->time_base = { 1, iccx->sample_rate };
	avcodec_open2(iccx, x_codec, nullptr);
	/*ifcx = avformat_alloc_context();
	AVCodec *input_codec = avcodec_find_decoder(AV_CODEC_ID_PCM_ALAW);
	auto strmOut = avformat_new_stream(ifcx, input_codec);
	iccx = strmOut->codec;

	iccx->channels = 1;
	iccx->channel_layout = av_get_default_channel_layout(iccx->channels);
	iccx->sample_fmt = input_codec->sample_fmts[0];
	iccx->sample_rate = 8000;
	iccx->bit_rate = 8000;
	iccx->time_base = { 1, iccx->sample_rate };

	avcodec_open2(iccx, input_codec, nullptr);*/
	///////////////////////////////////////////////////////////////////////////*/
	/*loggit("open_input");
	AVCodec *input_codec;
	int error;

...
CRTPReceive::CRTPReceive(vector<SHP_CConfPoint> callers, int ID) :callers_(callers), ID_(ID)
{
	...
	ext = Initer->data;
	outfile0.open("new0.mp3", std::ofstream::binary);
	outfile1.open("new1.mp3", std::ofstream::binary);
	outfile2.open("new2.mp3", std::ofstream::binary);
	outfile3.open("new3.mp3", std::ofstream::binary);
...
void CRTPReceive::receive_h(boost::system::error_code ec, size_t szPack, int i)
{
	...
	{
		string f = boost::to_string(callers_[i]->RawBuf.data);
		int sz = f.size();
		if (szPack > 12)
		{
...
int CRTPReceive::encode_audio_frame(AVFrame *frame, int *data_present, int i)
{
	...
	try
	{
		...
		if (i == 0) outfile0.write((char*)output_packet.data, output_packet.size);
		else if (i == 1) outfile1.write((char*)output_packet.data, output_packet.size);
		else if (i == 2) outfile2.write((char*)output_packet.data, output_packet.size);
		else if (i == 3) outfile3.write((char*)output_packet.data, output_packet.size);

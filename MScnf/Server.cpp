...
void InnerServer::Receive(boost::system::error_code ec_, size_t szPack_)
{
	if (szPack_ > 10)
	{
		...
		BOOST_LOG_SEV(lg, info) << "InnerServer::Receive(...): Received:\n"<<message.data;
		SHP_IPL ipl = make_shared<IPL>(IPL(message.data, message.sender));
		BOOST_LOG_SEV(lg, debug) << "InnerServer::Receive(...): parsed as:\n" << ipl->PrintAll();
...
void InnerServer::ReplyError(SHP_IPL ipl_)
{
	...
	BOOST_LOG_SEV(lg, fatal) << "InnerServer::ReplyError(...): result=\n" << result;
